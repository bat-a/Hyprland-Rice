...
install_dependencies() {
    case $DISTRO in
        ...
        "Arch Linux"|"Manjaro")
            ...
            # Remove the open-source Visual Studio Code (code) if installed
            if pacman -Qi code &>/dev/null; then
                echo "Removing conflicting 'code' package (open-source version)..."
                sudo pacman -Rns --noconfirm code || { echo "Failed to remove 'code' package. Exiting."; exit 1; }
            fi

            # Install visual-studio-code-bin (correct package for code-oss)
            echo "Installing Visual Studio Code (visual-studio-code-bin)..."
